...
import { FcConferenceCall, FcPhoneAndroid, FcFullTrash, FcCancel, FcCheckmark } from 'react-icons/fc';
import { IoMdStarOutline, IoMdStar } from 'react-icons/io';
import { RiQuillPenFill } from 'react-icons/ri';
import { toast } from 'react-toastify';

import { deleteContact, updateContact, updateFavorite } from 'redux/contacts/operations';
import {
  MobileContactInfo,
  MobileContactText,
  TableBtn,
...
const ContactItem = ({ contact }) => {
  ...
  const handleFavorite = () => dispatch(updateFavorite({ contactId: contact._id, favorite: !contact.favorite }));
  ...
  const handleConfirm = e => {
    ...
    dispatch(updateContact({ contactId: contact._id, name: newName, phone: newPhone }));
    ...
    <>
      <td>
        <Box display="flex" alignItems="center" gridGap={5}>
          ...
          <InputChange type="text" name="name" value={newName} onChange={e => setNewName(e.currentTarget.value)}/>
        </Box>
      </td>
      <td>
        <InputChange type="text" name="phone" value={newPhone} onChange={e => setNewPhone(e.currentTarget.value)}/>
      </td>
      <td>
      ...
      <td>
        <TableBtn type="button" onClick={e => setShowInput(!showInput)} cancelBtn>
          <FcCancel size={30} />
        </TableBtn>
      </td>
    </>
  ) : (
    <p>Here must be inputs for miniMob</p>
  ))
    :
    (isMobile ? (
    <>
      ...
      <td>
        <TableBtn type="button" onClick={() => setShowInput(!showInput)} changeBtn>
